...
ssize_t transfer_command_lcd(uchar cmd, uchar* parm, ssize_t parm_len);
...
static int open_lcd_dev()
{
    //return open("/dev/i2c-0", O_WRONLY);
    return open("/dev/i2c-0", O_RDWR);
}
static int close_lcd_dev(int fd)
{
    int ret = close(fd);
    if(0 != ret)
    {
 ...
 * are ignored.
 * */
void write_img_to_px_rect(uchar* img_buf, int img_px_w, int img_px_h, 
 ...
 * */
void write_img_to_pos(uchar* img_buf, int img_px_w, int img_px_h, int scrn_px_x, int scrn_px_y)
{
